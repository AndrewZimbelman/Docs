...
aridity_graph <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat)) +
      ...
      geom_point(aes(color = aridity), size = 0.5) +
      theme_minimal() + 
      scale_color_gradient(low = "blue", high = "yellow") + 
      labs(title = "Aridity of River Basins in United States" , x = "Latitude", y = "Longitude", color = 'aridity')
...
camels_aridity <- camels |> 
  arrange(-aridity)
pmean_graph <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat), color = "aridity") +
      borders("state", colour = "grey50") + 
      geom_point(aes(color = p_mean), size = 0.5) +
      theme_minimal() +
      scale_color_gradient(low = "yellow", high = "blue") + 
      labs( title = "Precipitation Levels in United States River Basins", x = "Latitude", y = "Longitude", scale = "free", color = "Mean Precipitation ")
...
```{r}
ggarrange(pmean_graph, aridity_graph,scale = 'free')
```
...
predicted_graph <- ggplot(pred_graph, aes(x = .pred, y =LogQmean, color = LogQmean))+
  scale_color_viridis_c() + 
  geom_point()+
  geom_smooth(method = lm) +
  labs( x = "Predicted Values", y = "Mean Daily Discharge",
        color = "Mean Discharge", title = "Predicted ")
observed_graph <- ggplot(camels, aes(x = low_prec_freq, y = LogQmean, color = LogQmean)) + 
         scale_color_viridis_c() + 
         geom_point() + 
         geom_smooth(method = lm) + 
          labs ( x = "Dry Day Drequency", y = "Mean Daily Discharge", color = "Daily Discharge" , title = "Observed")
ggarrange(predicted_graph,observed_graph, scale = 'free')
```
Overall, our observed values had a slightly steeper slope then the predicted values. I believe that the reason the predicted graph shows positive correlation is because it is visualizing the relationship and isn't able to show a negative relationship? I noticed this same issue in the lab 6 module. I believe that my model had some issue due to the skew in the high prec freq data, as it lowered the r squared value of the data, furthemore. I believe the r squared value would have increased and a better model would have been made if outliers were eliminated at the beginning of the model creation. filtering out these outlier values in the original graph could have helped. Overall, this was my first model that I've made and I feel like I have a better understanding about how model building works and I look forward to getting better at it as time goes on and better understanding the different models I can use. 
